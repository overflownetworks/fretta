...
if CLIENT then
	...
	SWEP.IconLetter = "B"
	killicon.AddFont( "sent_dynamite", "BT_KillIcons", SWEP.IconLetter, Color( 1, 177, 236, 255 ) )
...
function SWEP:Think()	
	...
	if self.Weapon:GetVar( "ExplodeTime", -1 ) != -1 and self.Weapon:GetVar( "ExplodeTime", -1 ) < CurTime() and SERVER then
		...
		if IsValid( self.Owner:GetBomb() ) then
		...
		util.BlastDamage( self.Weapon, self.Owner, self.Owner:GetPos(), 310, 140 )
