...
function GM:DrawCrosshair( )
	...
	if LocalPlayer():Alive() and not LocalPlayer():IsObserver() then
		...
		if (not hittraceres.HitWorld and IsValid(hittraceres.Entity)) then
...
hook.Add("HUDPaint", "VisualButtons", function()
...
surface.SetMaterial( OverlayMaterial )
	for _, ent in pairs( ents.FindInSphere( LocalPlayer():GetPos(), ButtonDistance ) ) do
		if IsValid(ent) and ent:GetClass() == "class C_BaseEntity" and
			...
			surface.DrawTexturedRect( pos2d.x - 64, pos2d.y - 64, 64, 64 )
